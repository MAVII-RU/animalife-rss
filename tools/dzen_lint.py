...
PATTERNS = [
    ...
    # --- СНИЖЕННАЯ ЛЕКСИКА (Дзен: «Сниженная лексика» → охват только подписчикам) ---
    (r"\bкал\b|\bкала\b|\bкалом\b|\bкале\b|\bкалу\b", 3, "сниженная лексика"),
    (r"какаш\w*|какает|какают|какал\w*", 4, "сниженная лексика"),
    (r"\bговн\w*|\bдерьм\w*|\bссыт\w*|\bсра[лтьн]\w*", 6, "сниженная лексика (груб./мат)"),
    (r"жрёт|жрет|жрут|сожрал\w*|нажрал\w*|обожрал\w*", 3, "сниженная лексика"),
    (r"блевот\w*|блюёт|блюет|блевал\w*|блёва", 3, "сниженная лексика"),
    (r"сдох\w*|подыхает|подох\w*|издыхает", 4, "сниженная лексика"),
    # --- ТЕМА БОЛЕЗНИ: Дзен блокирует МОНЕТИЗАЦИЮ по названию серьёзной/хирург. болезни,
    #     даже если текст про профилактику. Держи такие болезни вне заголовка/подзаголовков. ---
    (r"заворот желудк\w*|перекрут желудк\w*|\bGDV\b|\bЗЖ\b|острое вздути\w*", 4, "тема: тяжёлая болезнь (блок монетизации)"),
    (r"парвовирус|чумк[аи]\b|бешенств\w*|пироплазмоз|лептоспироз|панкреатит|пиометр\w*|онколог\w*|\bрак\b|опухол\w*", 3, "тема: болезнь (блок монетизации)"),
]
